...
public class DeportistaGUI extends JFrame implements ActionListener, MouseListener {
	...
	String[] columnas = new String[] { "Athlète", "Pays", "Discipline", "Modifier athlète", "Supprimer athlète" };
	...
	private JButton botonEditar = new JButton("Modifier");
	private JButton botonEliminar = new JButton("Supprimer");
	private JButton agregar = new JButton("+ Nouveau");
	private JButton exportar = new JButton("Exporter CSV");
	private JButton volver = new JButton("Retourner");

	public DeportistaGUI() {
		contador++;
		setTitle("Gestionnaire des Jeux Olympiques - Athlètes");
		setSize(700, 400);
	...
	private void cargarTabla() {
		...
		if (contador == 1) {
			JOptionPane.showMessageDialog(this,
					"Chargement des athlètes: Chaque athlète a une adresse électronique unique. Dans le cas où un athlète participe à plusieurs disciplines, l'adresse électronique correspondante doit être saisie.");
		}
	}				
		...
		if (e.getSource().equals(volver)) {
			...
			if (exportToCSV(tabla, directorioActual))
				JOptionPane.showMessageDialog(agregar,
						"Fichier CSV exporté avec succés à " + directorioActual);
		}
		...
		if (filas < tabla.getRowCount() && filas >= 0 && columnas < tabla.getColumnCount() && columnas >= 0) {
			...
			if (value instanceof JButton) {
				...
				if (botonClick.equals(botonEditar)) {
					...
					int respuesta = JOptionPane.showConfirmDialog(botonClick, "Êtes-vous sûr de vouloir supprimer cet athlète?",
